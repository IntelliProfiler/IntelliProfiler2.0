...
# --- Base output folder for saving (same location as the Male file) ---
...
# --- Fix the order of Group as Male → Female ---
data_all <- data_all %>%
  mutate(Group = factor(Group, levels = c("Male", "Female")))

# --- Create ZT numeric values, Day, and labels for ZT display ---
data_all <- data_all %>%
  mutate(
...
summary_data <- data_all %>%
  ...
  summarise(
    Avg_Distance = mean(Close_Contact_Ratio),
    SD = sd(Close_Contact_Ratio),
    .groups = "drop"
  )
...
global_max <- max(data_all$Close_Contact_Ratio, na.rm = TRUE)
...
# --- Generate colored data for significance asterisks (based on BH-adjusted p-values) ---
...
# --- Save ---
ggsave(
  file.path(out_dir, paste0("HourlyCCR_GroupComparison_", Sys.Date(), ".png")),
  p_group, width = 10, height = 4, dpi = 300, bg = "transparent"
)

ggsave(
  file.path(out_dir, paste0("HourlyCCR_GroupComparison_", Sys.Date(), ".pdf")),
...
summary_data_12h <- data_all_sum12h %>%
  ...
  summarise(
    Avg_CCR = mean(Close_Contact_Ratio),
    SD = sd(Close_Contact_Ratio),
    .groups = "drop"
  )
...
period_max_values <- summary_data_12h %>%
  ...
  summarise(max_y = max(Avg_CCR), .groups = "drop")
...
p_sum12h <- ggplot(summary_data_12h, aes(x = Index, y = Avg_CCR, color = Group, group = Group)) +
  ...
  geom_point(size = 1.5) +
  geom_ribbon(
    aes(ymin = Avg_CCR - SD, ymax = Avg_CCR + SD, fill = Group),
    alpha = 0.2, color = NA
  ) +
  scale_color_manual(values = color_values) +
  scale_fill_manual(values = color_values) +
...
# --- Save ---
ggsave(
  file.path(out_dir, paste0("Average12h_CCRGroupComparison_", Sys.Date(), ".png")),
  p_sum12h, width = 10, height = 4, dpi = 300, bg = "transparent"
)

ggsave(
  file.path(out_dir, paste0("Average12h_CCRGroupComparison_", Sys.Date(), ".pdf")),
