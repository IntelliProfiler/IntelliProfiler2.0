###########################################
# Script name : 17_IP_ZT_CCR_Heatmap.R
# Description : Generate 1-hour and 12-hour heatmaps of close contact ratio (CCR) without normalization across pairs and groups in Zeitgeber Time (ZT)
...
read_ccr_data <- function(file, group_name) {
  ...
  if ("CCR" %in% names(df)) {
    df <- df %>% mutate(CCR_Value = as.numeric(CCR))
  } else if ("Close_Contact_Ratio" %in% names(df)) {
    df <- df %>% mutate(CCR_Value = as.numeric(Close_Contact_Ratio))
  } else {
    stop("Column 'CCR' or 'Close_Contact_Ratio' was not found.")
  }
...
df_male   <- read_ccr_data(file_male, "Male")
df_female <- read_ccr_data(file_female, "Female")
...
out_dir <- file.path(dirname(file_male), "CCR_Heatmap_Output")
...
###########################################
# 1hr data
# CCR is already 0-100, so no normalization
###########################################
...
# Light = ZT0-11 mean
# Dark  = ZT12-23 mean
# CCR remains 0-100, so no normalization
###########################################
...
lightdark_all <- df_all %>%
  ...
  summarise(
    Mean_CCR = mean(CCR_Value, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
...
write.csv(
  hourly_all,
  file.path(out_dir, "CCR_Heatmap_1hr_All_Calc.csv"),
  row.names = FALSE
)

write.csv(
  lightdark_all,
  file.path(out_dir, "CCR_Heatmap_12hr_All_Calc.csv"),
  row.names = FALSE
)
...
write_xlsx(
  ...
  file.path(out_dir, "CCR_Heatmap_Calculation_Summary.xlsx")
)
...
plot_hourly_heatmap <- function(df, group_label, out_dir) {
  ...
  save_png_pdf(
    p,
    paste0("CCR_Heatmap_1hr_", group_label, "8"),
    out_dir
  )
}
...
plot_lightdark_heatmap <- function(df, group_label, out_dir) {
  ...
  save_png_pdf(
    p,
    paste0("CCR_Heatmap_12hr_", group_label, "8"),
    out_dir
  )
}
...
cat("✅ All CCR heatmaps and calculation tables have been exported to -> ", out_dir, "\n")