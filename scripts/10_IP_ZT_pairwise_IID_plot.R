###########################################
# Script name : 10_IP_ZT_pairwise_IID_plot.R
# Description : Perform group comparison of pairwise inter-individual distance (IID) across Zeitgeber Time (ZT) using linear mixed models and visualize results
...
library(lme4)
library(lmerTest)
emm_options(pbkrtest.limit = 5376)
...
message("▶ Please select the Male file")
file1 <- file.choose()
message("▶ Please select the Female file")
file2 <- file.choose()
group1_name <- "Male"
group2_name <- "Female"

# Output folder for saving (same location as Male file)
out_dir <- file.path(dirname(file1), "IID_Group_Comparison_Results")
if (!dir.exists(out_dir))
　dir.create(out_dir, recursive = TRUE)
message("Results will be stored in: ", out_dir)
...
fit_1h <- lmer(
  Avg_Inter_Individual_Distance ~ Group * ZT_Label + (1 | ID_1) + (1 | ID_2),
  data = data_all
)
...
write.csv(aov_res_1h, 
          file.path(out_dir, paste0("IID_ANOVA_TypeIII_1h_", Sys.Date(), ".csv")), 
          row.names = FALSE)
...
write.csv(result_table_1h,
          file.path(out_dir, paste0(
            "IID_Result_1h_BH_", Sys.Date(), ".csv"
          )),
          row.names = FALSE)
...
# Save
ggsave(
  file.path(out_dir, paste0("IID_GroupComparison_1h_", Sys.Date(), ".png")),
  p_group, width = 10, height = 4, dpi = 300, bg = "transparent"
)

ggsave(
  file.path(out_dir, paste0("IID_GroupComparison_1h_", Sys.Date(), ".pdf")),
  p_group, width = 10, height = 4, bg = "transparent"
)

message("Saved to: ", out_dir)

# 12-hour cumulative IID
...
# Analysis by Linear Mixed Model
fit_12h <- lmer(
  Avg_Inter_Individual_Distance ~ Group * Period_Label + (1 | ID_1) + (1 | ID_2),
  data = data_all_sum12h
)
...
write.csv(aov_res_12h, 
          file.path(out_dir, paste0("IID_ANOVA_TypeIII_12h_", Sys.Date(), ".csv")), 
          row.names = FALSE)
...
write.csv(result_table_12h,
          file.path(out_dir, paste0(
            "IID_Result_12h_BH_", Sys.Date(), ".csv"
          )),
          row.names = FALSE)
...
# PNG (transparent)
ggsave(
  file.path(out_dir, paste0("IID_GroupComparison_12h_", Sys.Date(), ".png")),
  p_sum12h, width = 10, height = 4, dpi = 300, bg = "transparent"
)

# PDF (transparent)
ggsave(
  file.path(out_dir, paste0("IID_GroupComparison_12h_", Sys.Date(), ".pdf")),
