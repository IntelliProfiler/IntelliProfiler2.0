###########################################
# Script name : 13_IP_ZT_IID_DLRatio.R
# Description : Calculate dark/light ratio (%) based on inter-individual distance (IID) across Zeitgeber Time (ZT) and perform group comparison using linear mixed models
...
out_dir_base <- file.path(dirname(file_male), "IID_DLR_Group_Comparison_Results")
...
read_group_data <- function(file, group_name) {
  read_excel(file) %>%
    mutate(
      Avg_Inter_Individual_Distance = as.numeric(Avg_Inter_Individual_Distance),
...
###########################################
# Calculate IID-based Dark/Light ratio
###########################################
...
df_ratio <- df_all %>%
  ...
  summarise(
    Mean_IID = mean(Avg_Inter_Individual_Distance, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_wider(
    names_from = LightDark,
    values_from = Mean_IID
  ) %>%
  mutate(
...
write.csv(
  df_ratio,
  file.path(out_dir_base, "IID_DLR_Calculated.csv"),
  row.names = FALSE
)
...
for (group_name in c("Male", "Female")) {
  ...
  for (pair_id in pair_levels_group) {
    ...
    p <- ggplot(df_plot, aes(x = Day, y = `Dark/Light (%)`, group = 1)) +
      ...
      scale_y_continuous(limits = c(0, y_max_each)) +
      labs(
        title = paste("Inter-indiv dist -", pair_id),
        x = "",
        y = "Dark/Light (%)"
      ) +
    ...
    save_png_pdf(
      p,
      paste0("IID_DLR_", group_name, "_", fn),
      out_dir
    )
  ...
  write.csv(
    df_group,
    file.path(out_dir, "IID_DLR_Each_ID.csv"),
    row.names = FALSE
  )
}
...
for (group_name in c("Male", "Female")) {
  ...
  p_all <- ggplot(df_g, aes(x = Day, y = `Dark/Light (%)`, color = ID, group = ID)) +
    ...
    scale_y_continuous(limits = c(0, y_max_each)) +
    labs(
      title = paste0("Inter-indiv dist - All IDs (", group_name, ")"),
      x = "",
      y = "Dark/Light (%)",
      color = "ID"
    ) +
...
write.csv(
  aov_res_ratio,
  file.path(out_dir_base, "IID_DLR_ANOVA_TypeIII.csv"),
  row.names = FALSE
)
...
write.csv(
  result_table_ratio,
  file.path(out_dir_base, "IID_DLR_Result_BH.csv"),
  row.names = FALSE
)
...
sig_stars <- result_table_ratio %>%
  ...
  mutate(y_pos = pmin(max_y + 40, 290))
