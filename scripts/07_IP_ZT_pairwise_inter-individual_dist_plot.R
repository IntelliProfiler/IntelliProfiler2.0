...
# --- Base output folder for saving (same location as Male file) ---
out_dir <- file.path(dirname(file1), "Group_Comparison_Results_revised")
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# --- Read data and assign group names ---
...
# --- Fix Group order to Male → Female ---
data_all <- data_all %>%
  mutate(Group = factor(Group, levels = c("Male", "Female")))

# --- Create ZT numeric values, Day, and ZT display labels ---
data_all <- data_all %>%
  mutate(
...
summary_data <- data_all %>%
  ...
  summarise(
    Avg_Distance = mean(Avg_Inter_Individual_Distance),
    SD = sd(Avg_Inter_Individual_Distance),
    .groups = "drop"
  )

# --- ANOVA & Tukey (for each ZT) ---
...
for (zt in zt_levels) {
  ...
  if (n_distinct(dat$Group) == 2) {
    model <- tryCatch(aov(Avg_Inter_Individual_Distance ~ Group, data = dat), error = function(e) NULL)
    if (!is.null(model)) {
...
# --- Set display labels every 3 hours ---
...
# --- Maximum value for each ZT ---
zt_max_values <- summary_data %>%
  group_by(ZT_Display) %>%
  summarise(max_y = max(Avg_Distance), .groups = "drop")

# --- Generate colored data for asterisks (based on BH-adjusted p-values) ---
...
p_group <- ggplot(summary_data, aes(x = ZT_Display, y = Avg_Distance, color = Group, group = Group)) +
  ...
  labs(
    title = "Inter-indiv dist - group comparison",
    x = "Day and ZT", y = "Average inter-indiv dist (cm)", color = "Group"
  ) +
...
# --- Save ---
ggsave(
  file.path(out_dir, paste0("Inter-Indiv Dist_GroupComparison_", Sys.Date(), ".png")),
  p_group, width = 10, height = 4, dpi = 300, bg = "transparent"
)

ggsave(
  file.path(out_dir, paste0("Inter-Indiv Dist_GroupComparison_", Sys.Date(), ".pdf")),
  p_group, width = 10, height = 4, bg = "transparent"
)

write.csv(
  anova_hourly,
  file.path(out_dir, paste0("ANOVA_Hourly_byZT_", Sys.Date(), ".csv")),
  row.names = FALSE
)

write.csv(
  tukey_hourly_df,
  file.path(out_dir, paste0("Tukey_Hourly_byZT_", Sys.Date(), ".csv")),
  row.names = FALSE
)

message("保存完了: ", out_dir)

# --- Global model (1hr) ---
model_global_1hr <- aov(Avg_Inter_Individual_Distance ~ Group * ZT_Display, data = data_all)

anova_global_1hr_df <- as.data.frame(summary(model_global_1hr)[[1]])
anova_global_1hr_df$Effect <- rownames(anova_global_1hr_df)
colnames(anova_global_1hr_df)[colnames(anova_global_1hr_df) == "Pr(>F)"] <- "p_value"
...
write.csv(
  anova_global_1hr_df,
  file.path(out_dir, paste0("ANOVA_Global_1hr_", Sys.Date(), ".csv")),
  row.names = FALSE
)

# --- Add Light/Dark information + create Period_Label ---
...
summary_data_12h <- data_all_sum12h %>%
  ...
  summarise(
    Avg_SD = mean(Avg_Inter_Individual_Distance),
    SD = sd(Avg_Inter_Individual_Distance),
    .groups = "drop"
  )

# --- Data for gray background ---
...
period_max_values <- summary_data_12h %>%
  ...
  summarise(max_y = max(Avg_SD), .groups = "drop")
...
# --- Function for converting x-axis labels ---
...
# --- 12h average graph ---
p_sum12h <- ggplot(summary_data_12h, aes(x = Index, y = Avg_SD, color = Group, group = Group)) +
  geom_rect(
  ...
  geom_point(size = 1.5) +
  geom_ribbon(
    aes(ymin = Avg_SD - SD, ymax = Avg_SD + SD, fill = Group),
    alpha = 0.2, color = NA
  ) +
  scale_color_manual(values = color_values) +
  scale_fill_manual(values = color_values) +
  ...
  labs(
    title = "Inter-indiv dist - group comparison",
    x = "", y = "Average inter-indiv dist (cm)", color = "Group"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
# --- Save ---
ggsave(
  file.path(out_dir, paste0("Ave12h_IIVGroupComparison_", Sys.Date(), ".png")),
  p_sum12h, width = 10, height = 4, dpi = 300, bg = "transparent"
)

ggsave(
  file.path(out_dir, paste0("Ave12h_IIVGroupComparison_", Sys.Date(), ".pdf")),
  p_sum12h, width = 10, height = 4, bg = "transparent"
)

write.csv(
  anova_12h,
  file.path(out_dir, paste0("ANOVA_Sum12h_byPeriod_", Sys.Date(), ".csv")),
  row.names = FALSE
)

write.csv(
  tukey_12h_df,
  file.path(out_dir, paste0("Tukey_Sum12h_byPeriod_", Sys.Date(), ".csv")),
  row.names = FALSE
)

# --- Global model (12hr) ---
model_global_12h <- aov(Avg_Inter_Individual_Distance ~ Group * Period_Label, data = data_all_sum12h)

anova_global_12h_df <- as.data.frame(summary(model_global_12h)[[1]])
anova_global_12h_df$Effect <- rownames(anova_global_12h_df)
colnames(anova_global_12h_df)[colnames(anova_global_12h_df) == "Pr(>F)"] <- "p_value"
...
write.csv(
  anova_global_12h_df,
  file.path(out_dir, paste0("ANOVA_Global_12hr_", Sys.Date(), ".csv")),
  row.names = FALSE
)