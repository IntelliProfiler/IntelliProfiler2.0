...
read_group_data <- function(file, group_name) {
  read_excel(file) %>%
    mutate(
      ...
      ID = as.character(ID),  # pair label (inter-individual distance pair ID)
      Group = group_name
    )
}
...
# --- Define pair levels and a fixed color palette (consistent across the whole dataset) ---
...
# --- Convert ID to a filename-safe string (replace '/', spaces, etc. with '_') ---
...
# --- Save plots for each individual pair (iterate only IDs present within each group) ---
for (group_name in c("Male", "Female")) {
  ...
  pair_levels_group <- sort(unique(df_group$ID))  # ★ important: group-specific IDs
  ...
  for (pair_id in pair_levels_group) {
    ...
    p <- ggplot(df_plot, aes(x = Day, y = `Dark/Light (%)`, group = 1)) +
      ...
      labs(title = paste0("Close contact ratio - ", group_name, " - ", pair_id),
           x = "", y = "Dark/Light (%)") +
    ...
    ggsave(file.path(out_dir, paste0("DarkLightRatio_", fn, ".png")),
           plot = p, width = 10, height = 4, dpi = 300)
    ggsave(file.path(out_dir, paste0("DarkLightRatio_", fn, ".pdf")),
...
# --- Plot all IDs (Male and Female) ---
for (group_name in c("Male", "Female")) {
  
  df_g <- df_all %>% filter(Group == group_name)
  ...
  p_all <- ggplot(df_g, aes(x = Day, y = `Dark/Light (%)`, color = ID, group = ID)) +
    ...
    scale_y_continuous(limits = c(0, y_max)) +
    labs(title = paste0("Close contact ratio - All IDs (", group_name, ")"),
  ...
  ggsave(file.path(out_dir_g, "DarkLightRatio_AllIDs.png"),
         plot = p_all, width = 10, height = 4, dpi = 300)
  ggsave(file.path(out_dir_g, "DarkLightRatio_AllIDs.pdf"),
...
p_group <- ggplot(summary_df, aes(x = Day, y = Avg, color = Group, group = Group)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2) +
  geom_ribbon(aes(ymin = Avg - SD, ymax = Avg + SD, fill = Group),
              alpha = 0.2, color = NA) +
  ...
  labs(title = "Close contact ratio",
...
sig_stars <- anova_df %>%
  ...
  mutate(
    ...
    y_pos = max(summary_df$Avg) + 10
  )

# --- Add significance stars to the group comparison plot ---
p_group <- p_group +
...
ggsave(file.path(out_dir_base, "DarkLightRatio_GroupComparison.pdf"),
       plot = p_group, width = 10, height = 4)
