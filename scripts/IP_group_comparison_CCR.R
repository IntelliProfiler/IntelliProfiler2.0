...
summary_data <- data_all %>%
  ...
  summarise(Avg_Distance = mean(Close_Contact_Ratio), SD = sd(Close_Contact_Ratio), .groups = "drop")
...
p_group <- ggplot(summary_data, aes(x = ZT_Display, y = Avg_Distance, color = Group, group = Group)) +
  ...
  scale_y_continuous(limits = c(0, 100)) +    # ← ★ set here ★
  ...
  labs(title = "Close contact ratio - group comparison",
...
summary_data_12h <- data_all_sum12h %>%
  ...
  summarise(Avg_CCR = mean(Close_Contact_Ratio), SD = sd(Close_Contact_Ratio), .groups = "drop")
...
p_sum12h <- ggplot(summary_data_12h, aes(x = Index, y = Avg_CCR, color = Group, group = Group)) +
  ...
  geom_ribbon(aes(ymin = Avg_CCR - SD, ymax = Avg_CCR + SD, fill = Group), alpha = 0.2, color = NA) +
